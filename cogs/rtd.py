...
class ReadTheDocs(commands.Cog):
    ...
    @commands.command(aliases=["rtd", "read", "see", "docs", "lookup"])
    @commands.guild_only()
    async def guide(
            self,
            ctx,
        ...
        """
        Gives you a link for a Photoshop user guide page.

        General functionalities are returned through an adobe.io endpoint.
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post('https://adobesearch-uss-enterprise.adobe.io/universal-search-enterprise/search',
                                    headers=headers,
                                    data=data
                                    ) as response:
